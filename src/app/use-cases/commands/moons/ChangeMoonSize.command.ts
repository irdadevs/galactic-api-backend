...
export class ChangeMoonSize {
  ...
  async execute(id: Uuid, dto: ChangeMoonSizeDTO): Promise<void> {
    ...
    const updated = await this.moonRepo.findById(id);
    if (updated) {
      await this.moonCache.invalidateForMutation(updated, previous);
      const planet = await this.planetRepo.findById(Uuid.create(updated.planetId));
      if (planet) {
