...
export class RequestAuditMiddleware {
  constructor(private readonly createLog: CreateLog) {}
    ...
    return (req: Request, res: Response, next: NextFunction) => {
      res.on("finish", () => {
        ...
        const shouldLog =
          ...
          status === 403 ||
          status === 429 ||
          ((req.method === "POST" || req.method === "PATCH" || req.method === "DELETE") && status < 400);
        ...
        const durationMs = Math.max(0, Date.now() - Number(res.locals.requestStartAt ?? Date.now()));
        void this.createLog.execute({
          source: "http",
          level,
          category,
          message,
